...
def deleteChat():
    ...
    try:
        ...
        if cursor.rowcount == 0:
            return jsonify({"error": "No chat found with the provided ID"}), 404
    except Error as e:
    ...
    return jsonify({"result": "Chat deleted successfully"}), 200
...
def saveChat():
    if request.method == 'POST':
        ...
        if not all([queries, ans1_mist, ans2_llama, user_id]):
        ...
        try:
            ...
            cursor.execute(
                'INSERT INTO chat_history (user_id, user_query, answer1, answer2, time_stamp) VALUES (%s, %s, %s, %s, %s)',
                (user_id, queries, ans1_mist, ans2_llama, timestamp)
            )
            connection.commit()
        except Error as e:
            app.logger.error(f"Error saving chat: {e}")
            return jsonify({"error": "An error occurred while saving the chat"}), 500
        finally:
            cursor.close()
            connection.close()

        return jsonify({"results_mistral": "done"}), 200
...
def login():
    ...
    if request.method == 'POST' and 'email' in request.form and 'password' in request.form:
        ...
        if connection is None:
            ...
            return render_template('login.html', msg=msg), 500

        try:
            cursor = connection.cursor(dictionary=True)
            cursor.execute(
                'SELECT * FROM user_account WHERE user_email = %s AND user_password = %s',
                (email, password)
            )
            ...
            if account:
                ...
                return render_template('main.html', msg='Logged in successfully!')
            else:
...
def signup():
    msg = ''
    if request.method == 'POST':
        if all(k in request.form for k in ('username', 'password', 'email')):
            ...
            if connection is None:
                msg = 'Database connection failed!'
                return render_template('signup.html', msg=msg), 500
